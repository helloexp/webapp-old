Feature: ACC231
  ...
  Scenario: Existing user with GUPick up at Uniqlo store, Gifting as Uniqlo, and Payment with Credit Card (ACC231)
    
    Given User goes to gu Cart
    When User goes to Login page
    When User logs in
      | email    | automation231@uniqlo.com |
      | password | test1234                 |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC231" to cart from mock page
