Feature: ACC111
  ...
  Scenario: Existing user with Pick up at Uniqlo store and Payment with Two Gift Card and Credit Card(ACC111)

    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation111@uniqlo.com |
      | password | test1234                 |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC111" to cart from mock page
    Given User goes to Cart
