Feature: ACC75
  ...
  Scenario: Existing user with Shipping Address, standard and Payment using one Gift Card and Credit Card (ACC75)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation75@uniqlo.com   |
      | password | test1234                 |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC75" to cart from mock page
    Given User goes to Cart
