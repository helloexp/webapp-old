Feature: ACC105
  ...
  Scenario: Existing user with yupacket using Pay At Store with Coupon (ACC105)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation105@uniqlo.com |
      | password | test1234                |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC105" to cart from mock page
    Given User goes to Cart
