Feature: ACC94
  ...
  Scenario: Existing user with nextday using Gift Card and Credit Card (ACC94)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation94@uniqlo.com |
      | password | test1234                |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC94" to cart from mock page
    Given User goes to Cart
