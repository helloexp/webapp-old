Feature: ACC77
  ...
  Scenario: Existing user with Shipping Address, standard and Payment using three Gift Cards and Credit Card (ACC77)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation77@uniqlo.com  |
      | password | test1234                 |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC77" to cart from mock page
    Given User goes to Cart
