Feature: ACC180
  ...
  Scenario: Existing user with Shipping Address and Deferred Payment with standard(ACC180)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation180@uniqlo.com |
      | password | test1234                 |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC180" to cart from mock page
    Given User goes to Cart
