Feature: ACC73
  ...
  Scenario: Existing user with Shipping Address, standard and Payment using three Gift Cards (ACC73)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation73@uniqlo.com |
      | password | test1234                 |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC73" to cart from mock page
