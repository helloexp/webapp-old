Feature: ACC104
  ...
  Scenario: Existing user with Yupacket using three Gift Card and Credit Card (ACC104)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation104@uniqlo.com |
      | password | test1234                |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC104" to cart from mock page
    Given User goes to Cart
