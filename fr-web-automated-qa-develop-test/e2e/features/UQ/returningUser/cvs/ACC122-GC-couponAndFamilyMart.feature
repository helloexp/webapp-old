Feature: ACC122
  ...
  Scenario: Existing user with CVS Family Mart, Coupon and Payment using one Gift Card and Credit Card (ACC122)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation122@uniqlo.com |
      | password | test1234                 |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC122" to cart from mock page
