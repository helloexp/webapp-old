Feature: ACC115
  ...
  Scenario: Existing user with cvs SevenEleven and Payment using two Gift Cards (ACC115)
    Given User goes to Cart
    When User goes to Login page
    When User logs in
      | email    | automation115@uniqlo.com |
      | password | test1234                 |

    Then User is successfully logged in
    When Prerequisites Reset user Cart data
    Given User adds products for "ACC115" to cart from mock page
