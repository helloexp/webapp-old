Feature: ACC205
  ...
  Scenario: New user with Pick up at Uniqlo store and Pay in Store(ACC205)
     Given User goes to Cart
    When  User goes to Login page
    When User decides to registers as a new user
    When User registers within the app using store account
    Then User confirms the new account
    When User returns to Cart
    Given User adds products for "ACC205" to cart from mock page
    Given User goes to Cart
